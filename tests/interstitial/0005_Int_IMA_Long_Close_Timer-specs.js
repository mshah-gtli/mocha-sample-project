...
// file name matching
describe('0005_Int_IMA_Long_Close_Timer-InProgress',function() {
    ...
    describe('Test for IMA Long Close Timer using ADSCore',function() {
    ...
    // COM2:Make easy use function for selection and verification of iframes            
        ...
        it( 'Should Select Ad Content Frame',function(done) {
            configutils.browser            
                .waitForElementByCss('#gadContainer iframe[name="ad_player_engine_'+gid+'"][style*="'+width+'"][style*="'+height+'"][style*="inherit"]').then(function(elem){
        ...
        it( 'Should Verify Header Message Text is Displayed Correctly',function(done) {
            configutils.browser
                .elementByCss('#ryInfoPanel #ryHeaderMessage').text()
                    .should .eventually .become('IMA Long Close Timer')
                .nodeify(done);
        });
    ...
    //NOTE: The Close button disabled state needs to be checked before checking the coutdown text   
        it( 'Should Verify the Close button is Present',function(done) {
            configutils.browser           
                .waitForElementByCssSelector('#ryInfoPanel #divClose.close', checkForDisplayed, _periodSeconds * 10)               
                .takeScreenshot()
                .nodeify(done);                        
        });
        ...
        it( 'Should Verify Ad Container is Hidden',function(done) {
            configutils.browser  
                .waitForElementByCss('#gadContainer[style*="hidden"]', 25 * _periodSeconds, _pollPeriodMs ).isDisplayed()
