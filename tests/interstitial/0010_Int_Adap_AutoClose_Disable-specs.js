...
// file name matching
describe('0010_Int_Adap_AutoClose_Disable-InProgress',function() {
    ...
    describe('Test for Adap AutoClose Disable using ADSCore',function() {
    ...
    // START GREENFRAME COPY SECTION
        ...
        it( 'Should Select Ad Content Frame',function(done) {
            configutils.browser            
                .waitForElementByCss('iframe#ad_player_engine_'+gid).then(function(elem){
       ...
       it( 'Should Return to Top iframe',function(done) {
        ...
        it( 'Should Verify the Countdown Text is Counting',function(done) {
            configutils.browser
                .elementByCss('#ryInfoPanel #divCountdown').text().then(validateText('#ryInfoPanel #divCountdown',15,1))
                .sleep(2 * _periodSeconds) // make sure that value changes
                .elementByCss('#ryInfoPanel #divCountdown').text().then(validateText('#ryInfoPanel #divCountdown',13,1))
                .nodeify(done);
        });
