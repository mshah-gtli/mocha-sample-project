...
// file name matching
describe('0009_Int_HTML5_Fallback_To_Flash-InProgress',function() {
    ...
    describe('Test for HTML5 Fallback To Flash using ADSCore',function() {
    ...
    // COM2:Make easy use function for selection and verification of iframes            
        ...
        it( 'Should Verify the Countdown Text is Counting',function(done) {
            configutils.browser
                .waitForElementByCssSelector('#ryInfoPanel #divCountdown', checkForDisplayed, _periodSeconds * 10)
        ...
        it( 'Should Verify Header Message Text is Displayed Correctly',function(done) {
            configutils.browser
                .elementByCss('#ryInfoPanel #ryHeaderMessage').text()
                    .should .eventually .become('This is HTML5 Player')
                .nodeify(done);
        });
    ...
    //NOTE: The Close button disabled state needs to be checked before checking the coutdown text   
        it( 'Should Verify the Skip button is Present',function(done) {
            configutils.browser
                .waitForElementByCssSelector('#ryContent #divSkip', checkForDisplayed, _periodSeconds * 10)                         
                .nodeify(done);                        
        });
        ...
        it( 'Should Verify Ad Container is Hidden',function(done) {
            configutils.browser  
                .waitForElementByCss('#gadContainer[style*="hidden"]', 35 * _periodSeconds, _pollPeriodMs ).isDisplayed()
    ...
    //TODO: NEED AN EXTRA TEST HERE TO CHECK IF THE CLOSE BUTTON IS ENABLED (requires a new close button id for enabled)    
    /*
        it( 'Should Verify the Video Object is Present',function(done) {
            configutils.browser
                .hasElementByCssSelector('#content object#Ads[data*="RyPlay.swf"]') // video 
                    .should .eventually .be .true
                .nodeify(done);
        });    
    */
