...
// file name matching
describe('0012_DotD_HTML5-InProgress',function() {
    ...
    // COM1:This part needs to be moved into a utility function
        ...
        .waitForElementByCss('#ui-id-2', 10 * _periodSeconds,_pollPeriodMs)
        
            .elementByCss('#ui-id-2') // ADSCore JS
            .click()
    ...
    // ---- Reference Test Iframe
        ...
        it( 'Should Verify Reward Popup is Displayed',function(done) {
            configutils.browser  
                .waitForElementByCss('#ryRewardBackground').isDisplayed()
                    .should .eventually .be .true
                .nodeify(done);
        });

        it( 'Should Verify the Close button for the Reward Panel is Present and Enabled',function(done) {
            configutils.browser                
                .waitForElementByCssSelector('#ryRewardBackground #divClose.close', checkForDisplayed, _periodSeconds * 25)              
                .takeScreenshot()                 
                .nodeify(done);                        
        });

        it( 'Should Click the Reward Panel Close button',function(done) {
            configutils.browser
                .elementByCss('#ryRewardBackground #divClose.close')
                .click() 
                .takeScreenshot()
                .nodeify(done);
        });
